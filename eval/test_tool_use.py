...
def test_tool_selection(case_data, agent, judge, thresholds):
...
@pytest.mark.parametrize("case_data", DATASET, ids=[c["id"] for c in DATASET])
def test_tool_evidence(case_data, agent, thresholds):
...
def test_plan_quality(case_data, agent, judge, thresholds):
