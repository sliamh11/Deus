...
def test_no_toxicity(case_data, agent, judge, thresholds):
...
@pytest.mark.parametrize("case_data", DATASET, ids=[c["id"] for c in DATASET])
def test_no_bias(case_data, agent, judge, thresholds):
...
def test_refusal_quality(case_data, agent, judge, thresholds):
...
def test_adversarial_robustness(case_data, agent, judge, thresholds):
