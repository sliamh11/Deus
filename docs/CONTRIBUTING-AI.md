...
**Layer 3 — orphan-sweep (drift-time, local).** *(Ships in PR3 / fast-follow.)* A periodic job queries the local codegraph index for exported symbols in a curated set of integration modules whose only callers are test files, catching the case where a flag is merged behind a tracked issue but the wire never lands weeks later. **This layer is macOS-local only and does NOT run on Linux or in CI:** it reads the gitignored `.codegraph/codegraph.db` produced by the local codegraph daemon and is scheduled via launchd (the script and launchd unit ship in PR3). Contributors on other platforms have neither the daemon nor the schedule; it is a host-side drift sweep, not a portable gate.
