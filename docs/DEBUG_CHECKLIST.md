...
# Expected: PID  0  com.deus (PID = running, "-" = not running, non-zero exit = crashed)

# 2. Any running containers?
container ls --format '{{.Names}} {{.Status}}' 2>/dev/null | grep deus

# 3. Any stopped/orphaned containers?
container ls -a --format '{{.Names}} {{.Status}}' 2>/dev/null | grep deus

# 4. Recent errors in service log?
grep -E 'ERROR|WARN' logs/deus.log | tail -20

# 5. Is WhatsApp connected? (look for last connection event)
grep -E 'Connected to WhatsApp|Connection closed|connection.*close' logs/deus.log | tail -5

# 6. Are groups loaded?
grep 'groupCount' logs/deus.log | tail -3
```
...
grep -E 'Container timeout|timed out' logs/deus.log | tail -10
...
grep -E 'Scheduling retry|retry|Max retries' logs/deus.log | tail -10
```
...
grep 'New messages' logs/deus.log | tail -10

# Check if messages are being processed (container spawned)
grep -E 'Processing messages|Spawning container' logs/deus.log | tail -10

# Check if messages are being piped to active container
grep -E 'Piped messages|sendMessage' logs/deus.log | tail -10

# Check the queue state — any active containers?
grep -E 'Starting container|Container active|concurrency limit' logs/deus.log | tail -10
...
grep -E 'Mount validated|Mount.*REJECTED|mount' logs/deus.log | tail -10

# Verify the mount allowlist is readable
cat ~/.config/deus/mount-allowlist.json
...
container run -i --rm --entrypoint ls deus-agent:latest /workspace/extra/
```
...
grep 'QR\|authentication required\|qr' logs/deus.log | tail -5
...
launchctl bootout gui/$(id -u)/com.deus

# Start the service
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.deus.plist
