...
- **`Done`, `merged`, and `deployed`/`applied` are three separate claims — never write or read one as proof of another.** Trigger: any time a ticket, handoff, or session log is about to say something shipped, is live, or is ready to build on — not only when clearing a pipeline dependency. Confirmed 2026-08-08 (LIA-519 roadmap): LIA-531 (credential-separation) merged and moved to `Done`, but its own PR body said "Design-only, implementation deferred to a future ticket" — a downstream session (LIA-536) nearly read "LIA-531 Done" as "activation-ready" for a dependent ruleset before catching the discrepancy by reading the merged PR's actual body, not just its Linear state. Recurred 2026-08-23 in two further shapes: a PR merged and marked `Done` whose own title said it still needed a supervised terraform apply (merged ≠ applied), and a migration merged with `Done` tickets while the columns it was meant to populate were NULL on 100% of rows (merged code ≠ populated data). Before treating any `Done`/merged item as clearing a dependency, as deployed, or as data-complete, read the merged PR's own description for scope caveats ("design-only," "implementation deferred," "deliberately not touched," "needs apply") and check the actual runtime/data surface, not just the tracker or Linear state.
