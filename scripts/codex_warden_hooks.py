...
def run_verification_gate(event: dict[str, Any], repo_root: Path) -> int:
    ...
    # Ambiguity check MUST run before any SHIP-cached fast path below --
    # otherwise a real SHIP for the cwd/override worktree would silently
    # cover a multi-target command that ALSO commits to a different,
    # unreviewed worktree (the exact regression "Fix C" closed; see
    # test_gate_blocks_on_ambiguous_multi_target_instead_of_riding_on_cwd_ship).
    cwd = Path(str(event.get("cwd") or os.getcwd())).resolve(strict=False)
    blocking = _blocking_ambiguity(event, repo_root, cwd)
    if blocking is not None:
        _block_pre_tool(_ambiguous_commit_block_message(blocking.distinct_targets))
...
def run_warden_backends_gate(role: str, event: dict[str, Any], repo_root: Path) -> int:
    ...
    cwd = Path(str(event.get("cwd") or os.getcwd())).resolve(strict=False)
    ambiguity = _blocking_ambiguity(event, repo_root, cwd)
    if ambiguity is not None:
        _block_pre_tool(_ambiguous_commit_block_message(ambiguity.distinct_targets))
        return 0
