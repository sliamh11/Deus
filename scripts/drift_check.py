...
Usage:
  ...
  python3 scripts/drift_check.py --codegraph-db    # validate codegraph DB schema matches what codegraph-cite-check queries
...
def _main_checkout_root(project_root: Path) -> Path:
    """Resolve the MAIN checkout root from *project_root*, which may itself be
    a linked worktree -- the repo's normal workflow. ``.codegraph/codegraph.db``
    only exists in the main checkout. Same derivation as ``warden-shim.sh``
    (``--git-common-dir`` with the trailing ``/.git`` stripped), so both agree
    on the same root by construction. Falls back to *project_root* if git fails
    (non-git dir, git missing).
    """
    common = _git_output(
        ["rev-parse", "--path-format=absolute", "--git-common-dir"], project_root
    )
    if not common:
        return project_root
...
def check_codegraph_db_schema(project_root: Path) -> int:
    """Validate the codegraph index still has the schema ``codegraph-cite-check``
    depends on (``run_codegraph_cite_check`` in ``codex_warden_hooks.py`` queries
    ``nodes.name``/``qualified_name``/``file_path`` directly; ``start_line``/
    ``end_line`` are part of the same table contract even though the cite-check
    itself doesn't query them, since a schema migration affecting one plausibly
    affects the others).

    The DB is gitignored and absent in CI and on a fresh machine that hasn't
    indexed yet -- SKIPs (returns 0) in that case, same posture as
    ``check_cache_map`` skipping when git is absent, so ``--all`` doesn't turn
    red on an infra gap this check can't itself fix. The ``files`` table is not
    a dependency here -- ``file:line`` citations are validated against the
    filesystem, not the index.

    Exit codes:
      0 -- schema OK, or DB absent/unreadable (SKIP)
      1 -- DB present but missing a required column
    """
    main_root = _main_checkout_root(project_root)
    db_path = main_root / ".codegraph" / "codegraph.db"
    if not db_path.is_file():
        print(
            f"SKIP: {db_path} not found -- codegraph-cite-check has no index "
            "to validate against"
        )
        return 0

    required = {"name", "qualified_name", "file_path", "start_line", "end_line"}
    try:
        ...
        print(f"SKIP: {db_path} unreadable ({exc}) -- cannot validate schema")
        return 0
    ...
    if missing:
        print(
            f"FAIL: {db_path}'s nodes table is missing column(s) {sorted(missing)} -- "
            "run_codegraph_cite_check (codex_warden_hooks.py) queries these "
            "directly; a schema change there needs a matching update to the query."
        )
        return 1

    print(
        f"OK: codegraph DB schema ({db_path}) has all columns codegraph-cite-check needs"
    )
    return 0
...
def check_all(project_root: Path, base_ref: str | None = None) -> int:
    ...
    print("\n=== codegraph DB schema ===")
    cg_rc = check_codegraph_db_schema(project_root)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="codegraph_db",
        help="Validate the codegraph index schema matches what codegraph-cite-check "
             "queries (nodes.name/qualified_name/file_path/start_line/end_line). "
             "Exits 0 if OK or the DB is absent (SKIP), 1 if a required column is missing.",
    )
    ...
    if args.codegraph_db:
        sys.exit(check_codegraph_db_schema(PROJECT_ROOT))
    elif args.cache_map:
