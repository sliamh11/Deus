...
RULES_OUTPUT = REPO_ROOT / "DEUS-RULES.md"
...
def render_patch(rows: list[dict], overrides: list[dict] | None = None) -> str:
    """Render the dsh patch with `- insert:` rows and top-level override entries.

    A bare row carrying an `id` is a REPLACE in dsh (targets an existing row in
    the composed tree and replaces its properties); `- insert:` ADDS new rows.
    Overrides are emitted after the insert block as separate patch entries.
    """
    lines = [
    ...
    for ovr in (overrides or []):
        lines.append(f"- id: {yaml_scalar(ovr['id'])}")
        if ovr.get("config"):
            lines.append("  config:")
            lines.extend(yaml_block(ovr["config"], 4))
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    if rules_files:
        ...
        overrides.append({
            ...
            "config": {
                "maxBytes": 65536,
                "instructionFileCandidates": ["AGENTS.md", "CLAUDE.md", "DEUS-RULES.md"],
            },
        })
        report.append(f"RULES {len(rules_files)} file(s) concatenated into {RULES_OUTPUT.name} ({RULES_OUTPUT.stat().st_size:,} bytes)")

    patch_path = args.out / "deus-dsh.patch.yml"
    patch_path.write_text(render_patch(rows, overrides), encoding="utf-8")
